...
class GenerateRegistrationsXlsJob < ApplicationJob
  ...
  def perform(event_id)
    ...
    sheet.update_row 0, "Datum", "Naam", "Email", "Studentnummer", "Ticket", "Comment", "Te betalen"
    event.registrations.each.with_index do |reg, i|
      sheet.update_row i + 1, reg.created_at, reg.name, reg.email, reg.student_number, reg.access_level.name, reg.comment, reg.to_pay
    end
